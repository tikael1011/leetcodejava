...
'''
instead of partition, we can use 'find', 'index', and find will return -1 if 'not found' while index will throw expection. 
and instad of list comprehensions, filter(lambda x: len(x) > 1, d.values())
'''

'''
for follow-up,

It really depends on the tree/storage structure. And I can't recognize when 'false positive' (or false) will happen
